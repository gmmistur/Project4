...
class Customer { 
    ...
    function create_record() { // display "create" form
        ...
        $this->generate_form_group("fileName", "", $this->fileName, "", "file");
        echo '<p>file description<p><textarea rows="10" cols="35" name="fileDescription"></textarea>';
    ...
    function insert_db_record () {
        if ($this->fieldsAllValid()) { // validate user input
            ...
            if ($result) {
                ...
                $this->password_hashed = MD5($this->password); // Hash password
                $sql = "INSERT INTO $this->tableName (name,email,mobile, password_hash, fileName, fileDescription, content) values(?, ?, ?, ?, ?, ?, ?)";
                $q = $pdo->prepare($sql);
                $q->execute(array($this->name, $this->email, $this->mobile, $this->password_hashed, $fileName, $fileDescription, $content));
    ...
    function list_records() {
        ...
        echo "
            ...
            <body>
                <a href='https://github.com/gmmistur/Project4' target='_blank'>*** Github link ***</a><br />
                <div class='container'>
                    <p class='row'>
                        <h3>$this->title" . "s" . "</h3>
                    </p>
        ...
        echo "
                    ...
                    <p class='row'>
                    <a href=/Prog04/uploads/> View all files uploaded using <strong>Option 1</strong></a>
                    ...
                    <div class='row'>
                        <table class='table table-striped table-bordered'>
                            ...
                            <tbody>";
        $sql = "SELECT content, fileDescription FROM $this->tableName ORDER BY id DESC";
        foreach ($pdo->query($sql) as $row) {
            echo "<tr>";
            echo "<td><img src='data:image/png;base64," . base64_encode($row['content'] ) . "' height='42' width='42'></td>";
        ...
        echo                "</tbody>
                    ...
                    <div class='row'>
                        <table class='table table-striped table-bordered'>
                            ...
                            <tbody>";
        $sql = "SELECT fileName, fileDescription FROM $this->tableName ORDER BY id DESC";
        foreach ($pdo->query($sql) as $row) {
            echo "<tr>";
            echo "<td><img src = 'uploads/" . $row["fileName"] . "' height='42' width='42'></td>";
            echo "<td>";
            if($row["fileName"] != null)
                echo "/Prog04/uploads/" . $row["fileName"] . "</td>";
            else
                echo "No image</td>";
            echo "<td>". $row["fileDescription"] . "</td>";
        }
